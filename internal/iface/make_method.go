...
// interfaceJumpDataLen 默认接口跳转数据长度
const interfaceJumpDataLen = 30

// MakeMethodCaller 构造 interface 的方法调用并放到 stub 区
// to 桩函数跳转到的地址
func MakeMethodCaller(to unsafe.Pointer) (uintptr, error) {
	placeholder, _, err := stub.TryAcquire(interfaceJumpDataLen)
	if err != nil {
		return 0, err
	}
	...
	if err := stub.CommitAcquire(uintptr(len(code))); err != nil {
		return 0, err
	}
