...
var testCases = []*TestCase{
	{
		...
		proxy: func(origin interface{}) interface{} {
			return func(i int) int {
				logger.Debug("proxy Caller called, args", i)
	...
	},
	{
		...
		proxy: func(origin interface{}) interface{} {
			var origin1 = origin
			return func(i int) Result {
				logger.Trace("proxy Caller9 called, args", i)
				originFunc, _ := origin1.(*func(i int) Result)
				return (*originFunc)(i)
			}
		},
...
func TestTestStaticProxy(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		// 静态代理函数
		patch, err := proxy.FuncName(basePath+"."+tc.funcName, tc.proxy(trampoline), trampoline)
		if err != nil {
...
func BenchmarkStaticProxy(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for _, tc := range testCases {
			...
			// 静态代理函数
			patch, err := proxy.FuncName(basePath+"."+tc.funcName, fun, trampoline)
			if err != nil {
